#! /bin/bash
# --- About: Encode input audio files with ffmpeg from current folder and save them in a folder named "opus"
...
mkdir -p opus && cd opus

parallel -j $threads 'f={}; ffmpeg -i ../{} -c:a libopus -b:a 224K -vbr on "${f%."'${ext}'"}.ogg"' < "../$rnd.txt" && rm -f "../$rnd.txt" && echo 'Done!'
